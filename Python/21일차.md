...
- 객체(object): 속성을 가지고 있는 대상.    # 붕어빵
- 클래스(class): 객체를 만들기 위한 설계도(코드).    # 붕어빵 기계
  - 속성(property): 객체가 가져야 하는 값(들). 변수.
  - 기능(method): 클래스 안에서 선언하는 함수.
    - 메서드를 선언할 때 첫번째 아규먼트 self
- 인스턴스(instence): 메모리에 생성된 객체.    # 만들어진 붕어빵 한개한개
...
class Student:
    ...
    # 매서드(method)
    def get_total(self):    # self는 Student 객체를 의미미
...
print(student1)
print(student1.id)    # student1이 가지고 있는 id
print(student1.name)
print(student1.korean)
print(student1.get_total())    # 생성된 객체에서 메서드 호출. self에 student1가 들어간다고 생각
