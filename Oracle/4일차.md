...
- self-join: 같은 테이블에서 join을 하는 방법<br>
- 사번, 직원이름, 매니저 사번, 매니저 이름 검색<br>
- ANSI inner join<br>
```sql
...
order by e1.empno;
```
![image](https://github.com/user-attachments/assets/32f34aff-bc32-4ec6-a3c1-b1fa3e4ddff1)
...
```
-> 그냥 조인했을 때<br>
![image](https://github.com/user-attachments/assets/bd9f6727-20aa-4e26-8934-687120c49580)

- Ex1. 직원 이름, 근무 도시를 검색. 도시 이름으로 정렬.(정렬은 기본적으로 오름차순)
  - inner join
  ```sql
...
- Ex2. 직원 이름, 매니저의 이름, 직원 급여, 직원 급여 등급 검색<br>
- 정렬 순서: (1) 매니저 (2) 급여 등급, join 두 번
  ```sql
...
- Ex3. 직원 이름, 부서 이름, 급여, 급여 등급 검색<br>
- 정렬 순서: (1) 부서 이름 (2) 급여 등급, 3개의 테이블 조인
```sql
...
- Ex4. 부서 이름, 부서 위치, 부서의 직원수 검색. 부서 번호 오름차순(group by 필요)
```sql
select d.dname, d.loc, count(e.empno) → d.dname를 출력하고 싶은데 통계함수가 아니라서 출력이 안되니깐
from emp e
  right join dept d on e.deptno = d.deptno
...
- Ex5. 부서 번호, 부서 이름, 부서 직원 수, 부서의 급여 최솟값/최댓값 검색<br>
- 부서 번호 오름차순(group by 필요)
```sql
select d.deptno, d.dname, count(e.empno), min(e.sal), max(e.sal) # 그룹으로 묶은 컬럼만 선택 가능
from emp e
join dept d on e.deptno = d.deptno
...
- Ex6. 부서 번호, 부서 이름, 사번, 직원 이름, 매니저 사번, 매니저 이름, 직원 급여, 직원 급여 등급 검색<br>
- 급여가 2,000 이상인 직원들만 검색(where절 필요)<br>
- 정렬 순서: (1) 부서 번호 (2) 사번<br>
```sql
select d.deptno, d.dname, e1.empno, e1.ename,
  e2.empno as “매니저 사번”, e2.ename as “매니저 이름”, e1.sal, s.grade
from dept d
  join emp e1 on d.deptno = e1.deptno
  join emp e2 on e1.mgr = e2.deptno
  join salgrade s on e1.sal between s.losal and s.hisal
where e1.sal >= 2000
order by d.deptno, e1.empno;
```
![image](https://github.com/user-attachments/assets/b74f9522-680c-4d92-b22c-2af0e6c479da)
